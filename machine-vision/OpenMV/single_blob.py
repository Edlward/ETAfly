...
# 色块检测数据打包
def pack_blob_data(blob,ctrl,flag):

    #---包头------功能位----数据个数位---------------------------------数据包------------------------------------------------累加和校验
    #【AA 55】    【01】     【0x0A】      【阈值、色块个数、识别度、简单距离、色块中心坐标(高8位x，低8位x,高8位y，低8位y)、res】     【sum】
...
def single_blob(img,ctrl,thresholds,threshold_index,uart):
    ...
    if blobs:
        ...
        uart.write(pack_blob_data(max_blob,ctrl,1))
        LEDG.on()
    else:

        LEDG.off()
        uart.write(pack_no_blob_data(ctrl,0))
